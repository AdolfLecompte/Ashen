...
if [[ $DO_STOW -eq 1 ]]; then
    ...
    if ! command -v stow >/dev/null; then
        ...
        # A link stow did not make itself -- one written by hand, or with an
        # absolute path -- counts as "not owned by stow" and aborts the WHOLE
        # transaction, so an update would silently change nothing. Any link that
        # already points into the repo is ours to redo: drop it and let stow lay
        # it again. Only links are touched; a real file is left alone and stow is
        # allowed to complain about it.
        drop_our_links() {
            local pkg="$1" src target
            while IFS= read -r -d '' src; do
                target="$HOME/${src#"$REPO_DIR/$pkg/"}"
                [[ -L "$target" ]] || continue
                case "$(readlink -f "$target" 2>/dev/null)" in
                    "$REPO_DIR"/*) rm -f "$target" ;;
                esac
            done < <(find "$REPO_DIR/$pkg" -mindepth 1 -print0)
        }

        say "Symlinking configs with stow..."
        # One package at a time, for the same reason the package install retries
        # one by one: a single conflict must not take the other eight with it.
        for pkg in "${STOW_PKGS[@]}"; do
            drop_our_links "$pkg"
            ( cd "$REPO_DIR" && stow -R -t "$HOME" "$pkg" ) 2>/dev/null \
                || warn "stow: $pkg has a conflict — move the offending file aside and re-run"
        done
    fi
else
...
#
# This runs on EVERY setup, not only when the file is missing: an update brings
# new templates, and a machine that already had a gtk.css would otherwise keep
# the one generated by the old ones.
if command -v matugen >/dev/null; then
    say "Regenerating the GTK palette..."
    if "$REPO_DIR/scripts/ashen-recolor.sh" >/dev/null 2>&1 \
       && [[ -f "$HOME/.config/gtk-3.0/gtk.css" ]]; then
        ok "GTK palette regenerated from the current wallpaper"
    else
        ...
        if [[ -n "$wall" ]]; then
            ...
            matugen image "$wall" --mode "$mode" --source-color-index 0 >/dev/null \
                && "$REPO_DIR/scripts/ashen-accent.sh" >/dev/null 2>&1 \
                && ok "GTK palette generated from $(basename "$wall")"
        else
...
# used: it re-runs itself under sudo (which a hardened sudoers refuses) and it
# only knows the colours Papirus ships.
if [[ -d /usr/share/icons/Papirus ]]; then
    say "Rebuilding the accent folders..."
    "$REPO_DIR/scripts/ashen-folders.sh" >/dev/null 2>&1 \
        && ok "Folder theme ready (turn it on in Settings > Appearance)"
fi
